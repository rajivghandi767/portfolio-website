...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for idx, proj_data in enumerate(projects):
            project, created = Project.objects.update_or_create(title=proj_data["title"], defaults=proj_data)
            if created:
                if idx == 0:
        ...
        for idx, post_data in enumerate(posts):
            post, created = Post.objects.update_or_create(title=post_data["title"], defaults=post_data)
            if created:
                if idx == 0:
        ...
        for card_data in cards:
            Card.objects.update_or_create(card_name=card_data["card_name"], defaults=card_data)
